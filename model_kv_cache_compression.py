...
class Transformer(nn.Module):
    ...
    def setup_caches(self, max_batch_size, max_seq_length, prompt_size=None):
        ...
        for b in self.layers:
            b.attention.kv_cache = self.kv_cache_class(self.max_batch_size, self.max_seq_length, self.config.n_local_heads, self.config.head_dim, dtype=self.output.weight.dtype)
...
class TransformerAttentionSink(Transformer):
    def __init__(self, config: ModelArgs, sliding_window: int = 8, global_tokens: int = 2) -> None:
    ...
    def setup_caches(self, max_batch_size, max_seq_length, prompt_size=None):
        ...
        if hasattr(self.output, "scales"):
            ...
            dtype = self.output.scales_and_zeros.dtype
        for b in self.layers:
            b.attention.kv_cache = self.kv_cache_class(self.max_batch_size, self.max_seq_length, self.config.n_local_heads, self.config.head_dim, dtype=self.output.weight.dtype, global_tokens=self.global_tokens, sliding_window=self.sliding_window)
...
class TransformerL2Norm(Transformer):
    def __init__(self, config: ModelArgs, keep_ratio: float = 1.0, prune_after: int = 1024) -> None:
        super().__init__(config)
    ...
    def setup_caches(self, max_batch_size, max_seq_length, prompt_size=None):
        ...
        if hasattr(self.output, "scales"):
            ...
            dtype = self.output.scales_and_zeros.dtype
        for b in self.layers:
            b.attention.kv_cache = self.kv_cache_class(self.max_batch_size, self.max_seq_length, self.config.n_local_heads, self.config.head_dim, dtype=self.output.weight.dtype, keep_ratio=self.keep_ratio, prune_after=self.prune_after)
