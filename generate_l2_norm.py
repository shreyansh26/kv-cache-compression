...
from model_l2_norm import Transformer
...
def prefill(model: Transformer, x: torch.Tensor, input_pos: torch.Tensor, **sampling_kwargs) -> torch.Tensor:
    # input_pos: [B, S]
    mask = create_block_mask(causal_mask, 1, 1, input_pos.shape[0], model.max_cache_size, device=x.device)
...
def decode_one_token(model: Transformer, x: torch.Tensor, input_pos: torch.Tensor, block_mask: BlockMask, **sampling_kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    mask.seq_lengths = (1, model.max_cache_size)
    logits = model(mask, x, input_pos)
...
def _load_model(checkpoint_path, device, precision, use_tp, keep_ratio, prune_after):
    use_cuda = 'cuda' in device
    with torch.device('meta'):
        model = Transformer.from_name(checkpoint_path.parent.name, keep_ratio=keep_ratio, prune_after=prune_after)
...
def main(
    ...
    print("Loading model ...")
    t0 = time.time()
    model = _load_model(checkpoint_path, device, precision, use_tp, keep_ratio, prune_after)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_new_tokens', type=int, default=400, help='Maximum number of new tokens.')
    ...
    main(
        ...
        args.speculate_k, args.device, args.keep_ratio, args.prune_after
    )
